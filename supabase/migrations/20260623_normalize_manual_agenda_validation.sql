-- Ubah nomor agenda surat masuk dan surat keluar menjadi nomor urut otomatis:
-- 1, 2, 3, 4, ... berdasarkan urutan input data.

create or replace function public.normalize_manual_agenda_no(p_value text)
returns text
language sql
immutable
as $$
  select case
    ...
    when btrim(p_value) ~ '^[0-9]+$' then nullif(regexp_replace(btrim(p_value), '^0+', ''), '')
    when btrim(p_value) ~ '.*([0-9]+)$' then nullif(regexp_replace(substring(btrim(p_value) from '([0-9]+)$'), '^0+', ''), '')
    else null
  end
$$;
...
begin
  ...
  update public.incoming_letters i
     set agenda_no = '__tmp_in_' || numbered.next_agenda_no,
  ...
  update public.outgoing_letters o
     set agenda_no = '__tmp_out_' || numbered.next_agenda_no,
         updated_at = timezone('utc', now())
    from numbered
...
begin
  if new.agenda_no is null or btrim(new.agenda_no) = '' then
    new.agenda_no := nextval('public.incoming_agenda_seq')::text;
  end if;
  return new;
end;
$$;
...
begin
  if new.agenda_no is null or btrim(new.agenda_no) = '' then
    new.agenda_no := nextval('public.outgoing_agenda_seq')::text;
  end if;
  return new;
end;
$$;
...
create trigger trg_assign_incoming_agenda_no
before insert on public.incoming_letters
for each row execute function public.assign_incoming_agenda_no();

drop trigger if exists trg_assign_outgoing_agenda_no on public.outgoing_letters;
create trigger trg_assign_outgoing_agenda_no
before insert on public.outgoing_letters
for each row execute function public.assign_outgoing_agenda_no();
...
begin
  ...
  if nullif(p_payload->>'id', '') is null then
    v_agenda_no := nextval('public.incoming_agenda_seq')::text;
    begin
...
begin
  ...
  if nullif(p_payload->>'id', '') is null then
    v_agenda_no := nextval('public.outgoing_agenda_seq')::text;
    begin
